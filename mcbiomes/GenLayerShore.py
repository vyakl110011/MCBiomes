...
class GenLayerShore(Main):
        ...
        for i in range(aH):
            for j in range(aW):
                ...
                if k == 14:
                    ...
                    if (
                        self.isJungleCompatible(i2)
                        and self.isJungleCompatible(l2)
                        and self.isJungleCompatible(k3)
                        and self.isJungleCompatible(j4)
                    ):
                        if (
                            not self.isBiomeOceanic(i2)
                            and not self.isBiomeOceanic(l2)
                            and not self.isBiomeOceanic(k3)
                            and not self.isBiomeOceanic(j4)
                        ):
                            aint1[j + i * aW] = k
                        else:
                    ...
                    if self.isValidId(k) and self.isSnowy(k):
                        self.replaceIfNeighborOcean(
                            aint, aint1, j, i, aW, k, 26
                        )  # cold beach
                        ...
                        if not (k in [0, 24, 6, 7]):  # not a ocean or swamp or river
                            ...
                            if (
                                not self.isBiomeOceanic(l1)
                                and not self.isBiomeOceanic(k2)
                                and not self.isBiomeOceanic(j3)
                                and not self.isBiomeOceanic(i4)
                            ):
                                aint1[j + i * aW] = k
                            else:
                                aint1[j + i * aW] = 16  # beach
                        ...
                        if (
                            not self.isBiomeOceanic(l)
                            and not self.isBiomeOceanic(i1)
                            and not self.isBiomeOceanic(j1)
                            and not self.isBiomeOceanic(k1)
                        ):
                            if (
                                self.isMesa(l)
                                and self.isMesa(i1)
    ...
    def isJungleCompatible(self, id):
        if self.isValidId(id) and self.sameClass(id, 21):
            ...
            return id in [23, 22, 21, 4, 5] or self.isBiomeOceanic(
                id
            )  # jungle biomes or taiga or forest or oceanic
